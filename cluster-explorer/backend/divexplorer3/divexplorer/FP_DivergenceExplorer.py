...
def oneHotEncoding(dfI, include_absence=True):
    ...
    if not include_absence:
        for attribute in attributes:
            X_one_hot.drop(columns=[f"{attribute}=0"], inplace=True)
...
class FP_DivergenceExplorer:
    def __init__(
        ...
        self.X = oneHotEncoding(X_discrete.drop(columns=cols), include_absence)
